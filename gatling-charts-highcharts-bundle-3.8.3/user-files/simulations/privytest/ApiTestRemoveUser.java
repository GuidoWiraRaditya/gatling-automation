...
public class ApiTestRemoveUser extends Simulation {

  {
    ...
    ScenarioBuilder scn = scenario("RemoveUser")
      ...
      .pause(5)
      .exec(
        http("happy - User deleted")
          .post("/api/v1/register/remove")
          .headers(headers_0)
          .formParam("phone", phoneNumber)
          .check(
            status().is(201),
            jsonPath("$.data").is("Success remove user " + phoneNumber)
          )
      )
      .exec(
        http("negative - User not found")
        .post("/api/v1/register/remove")
        .headers(headers_0)
        .formParam("phone", phoneNumber)
        .check(
          status().is(500)
          //status is 500 if we check it manually
